...
import { ORIGINAL_IMAGE_BASE_URL, SMALL_IMG_BASE_URL } from '../utils/constants.js';
        ...
        <div className='mx-auto container px-4 py-8 h-full'>
            ...
            <div classname='aspect-video mb-8 p-2 sm:px-10 md:px-32'>
                {trailers.length > 0 && (
                    ...
                    width={"100%"}
                    height={"70vh"}
                    className='mx-auto overflow-hiddn rounded-lg'
                    url={`https://www.youtube.com/watch?v=${trailers[currentTrailerIdx].key}`} />
                )}

                {trailers?.length === 0 && (
                    <h2 className= 'text-xl text-center mt-5'>No Trailers Available for {" "}
            ...
            {/* movie details */}
            <div className='flex flex-col mt-40 md:flex-row items-center justify-between gap-20 max-w-6xl mx-auto'>
                ...
                <img 
                    src={ORIGINAL_IMAGE_BASE_URL + content?.poster_path}
                     alt="Poster Image" className='max-h-[600px] rounded-md'></img>
            </div>
            ...
            {similarContent.length > 0 && (
                <div className='mt-12 max-w-5xl mx-auto relative'>
                    ...
                    <div className='flex overflow-x-scroll scrollbar-hide gap-4 pb-4 group' ref={sliderRef}>
                        ...
                        opacity-0 group-hover:opacity-100 transition-all duration-300 cursor pointer bg-red-600 text-white rounded-full' onClick={scrollRight} />

                        <ChevronLeft className='absolute top-1/2 -translate-y-1/2 left-2 w-8 h-8
                        opacity-0 group-hover:opacity-100 transition-all duration-300 cursor pointer bg-red-600 text-white rounded-full' onClick={scrollLeft} />
                    </div>
                </div>
            )}
        </div>
